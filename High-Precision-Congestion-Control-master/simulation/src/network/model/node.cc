...
bool Node::SwitchReceiveFromDevice(Ptr<NetDevice> device, Ptr<Packet> packet, MyCustomHeader &ch){
	NS_ASSERT_MSG(false, "Calling SwitchReceiveFromDevice() on a non-switch node or this function is not implemented");
}
...
void Node::MatchSharedTableSendToRelatedSender(Ptr<NetDevice> device, Ptr<Packet>p, MyCustomHeader &ch) {
	NS_ASSERT_MSG(false, "Calling MatchSharedTableSendToRelatedSender() on a non-switch node or this function is not implemented");
}
} // namespace ns3
